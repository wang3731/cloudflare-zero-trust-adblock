...
def delete_existing_adguard_policy():
    """先刪除引用 list 的 Gateway Rule，否則 list 無法刪除"""
    url = f"{BASE_URL}/gateway/rules"
    response = requests.get(url, headers=HEADERS)
    if response.status_code != 200:
        print(f"⚠️  無法取得 Gateway Rules: {response.status_code}")
        return
...
Examples:
  ...
  python3 upload_to_cloudflare.py --dry-run
    ...
    # 改為
    delete_existing_adguard_policy()          # ← 先刪 policy
    time.sleep(2)                             # ← 等 API 生效
    delete_existing_adguard_lists(args.auto_approve)  # ← 再刪 lists
