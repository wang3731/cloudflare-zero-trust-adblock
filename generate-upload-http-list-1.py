...
POLICY_NAME = "AdGuard_HTTPS_No_Inspect"
LIST_NAME_PREFIX = "AdGuard_HTTPS_No_Inspect"
...
def main():
    ...
    for i, (chunk, csv_path) in enumerate(zip(chunks, csv_files), 1):
        list_name = f"{LIST_NAME_PREFIX}_Part_{i}"
        print(f"\n[{i}/{total}] {list_name}")

        list_id = create_gateway_list(
            name=list_name,
            description=f"AdGuard_HTTPS_exclusions_Part_{i}_of_{total}",
        )
